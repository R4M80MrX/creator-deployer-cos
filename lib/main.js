...
module.exports = function(config) {
  ...
  let publicDir = path.resolve(rootDir, "build", "web-mobile");
  ...
  if (!cfgs.cdnEnable) {
    ...
    getFiles(publicDir, file => {
      if (file.match(cfgs.cdnConfig.folder)) {
        ...
        localImgsMap.set(
          getUploadPath(file).replace(cfgs.cdnConfig.folder + "/", ""),
  ...
  function cosStart(fileMap, cfgs, cdn) {
    ...
    console.log(
      chalk.cyan(
        "本地文件准备就绪，正在从" + cfgs.bucket + "获取远程文件列表.."
      )
    );
    const cos = new COS({
    ...
    return getCosFiles(cos, cfgs)
      .then(cosFileMap => {
        if (cosFileMap.size > 0) {
          console.log(
            chalk.cyan("获取远程文件成功，开始对比本地文件和远程文件..")
          );
        } else {
      ...
      .then(allFiles => {
        return deleteFile(cos, cfgs, allFiles.extraFiles)
          .then(function(data) {
            if (data != false) {
              console.log(
                chalk.cyan("删除远程多余文件成功，开始上传本地文件..")
              );
            }
            return allFiles;
          })
  ...
  return cosStart(localFileMap, cfgs, false).then(function() {
...
function checkConfigs(config) {
  ...
  if (
    ...
    let tips = [
      ...
      "请检查根目录下的 deploy.js 文件中是否设置了以下信息",
      ...
      "",
      "您还可以访问插件仓库，以获取详细说明： " +
        chalk.underline("https://github.com/75k/hexo-deployer-cos-enhanced")
    ];
    ...
    if (!cfgs.cdnConfig) {
      ...
      if (!cfgs.cdnConfig.enable) {
        ...
        if (
          !cfgs.cdnConfig.cdnUrl ||
          !cfgs.cdnConfig.bucket ||
          !cfgs.cdnConfig.region ||
          !cfgs.cdnConfig.folder ||
          !cfgs.cdnConfig.secretId ||
          !cfgs.cdnConfig.secretKey
        ) {
          let tips = [
          ...
          cfgs.cdnEnable = true;
          cfgs.cdnConfig.cdnUrl = cfgs.cdnConfig.cdnUrl.replace(
            /([^\/])$/,
            "$1/"
          );
          return cfgs;
